<%--
  ...
  Date: 2016/8/3
  Time: 11:09
  describtion:后台管理--【拼车信息会】乘客微信自行发布信息列表
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('.wx_passenger_departure_list').addClass('active_li');
    // 绑定键盘按下事件
  ...
  var click_type;
  var user_id;
  var action_url = "/api/db/passenger/departure";
  ...
  function loadWXUser(){
    var obj={};
    obj.action='passenger_departure_count';
...
//      var destination = global_data.data[i].destination;//目的小城市
      ...
      var user_avatar = global_data.data[i].user_avatar;//微信头像
      var user_name = global_data.data[i].user_name;//微信昵称
      ...
      addUserDisplay(user_avatar,user_name,insert_time,departure_city, destination_city,
              description, booking_seats,id, publish_time,boarding_point,breakout_point);
      if (boarding_point == "") {
        $($('.begin_city')[i]).hide();
      }
      if (breakout_point == "") {
...
//        user_name="佚名";
//      }
//      if(user_avatar=="" || user_avatar==undefined){
//        user_avatar="/resource/images/icon_people.png";
//      }
      //进行非空判断
  ...
  //添加用户列表
  function addUserDisplay(user_avatar,user_name,insert_time,departure_city, destination_city,
                          description, booking_seats,id, publish_time,boarding_point,breakout_point) {
    $(".not_find_message").before('<li class="drive_li_container" index=' + id + ' >' +
            ...
            '<span class="begin_city" style="color:#999794;font-size: 12px;"><i class="circle"></i>' + boarding_point + '</span>' +
            '<span class="driver_route_span" style="color: #F5AD4E;">——</span>' +
            '<span class="driver_route_span driver_route_end">' + destination_city + '</span>' +
            '<span class="end_city" style="color:#999794;font-size: 12px;"><i class="circle"></i>' + breakout_point + '</span>' +
            '</div>' +
            ...
            '<span class="driver_type_span driver_type_seat">乘客数量' + booking_seats + '个</span>' +
            '</div>' +
...
//            '<span class="driver_detail_option_edit" onclick="toEdit(this)">编辑</span>' +
  ...
  function toHref(obj){
    ...
    window.location.href="http://pinchenet.com/laihui/passenger/order_info?id=" + li_index;
  }
  //生成文字信息
  function addManagerStyle(obj) {
    ...
    $('.find_href_span').text("http://pinchenet.com/laihui/passenger/order_info?id=" + driving_id);
