...
//        搜索重置page
  ...
  function updateManage() {
    ...
    obj.id=user_id;
    obj.route_price=cost;
    operation.operation_ajax(action_url,obj,findMessage);
    removeManagerStyle();
  }
  ...
  function checkInputMessage(){
    if($('.route_start').val()==""){
      ...
      showErrorTips("请输入参考价")
    }else if(isNaN($('.route_money').val())){
      showErrorTips("输入格式不正确")
    }else{
      if (click_type == 1) {
        addManage()
      } else {
  ...
  function insertUserMessage(){
    ...
    for (var i = 0; i < global_data.data.length; i++){
      var all_driver_count=global_data.data[i].all_driver_count;//所有的司机
      var today_driver_count=global_data.data[i].today_driver_count;//今天的司机数量
      ...
      var all_info_count=global_data.data[i].all_info_count;//发布信息
      var today_info_count=global_data.data[i].today_info_count;//发布信息
      var cost=global_data.data[i].route_price;//参考价
      if(cost =="" || cost == undefined){
        cost="未定"
      }

      addUserDisplay(cost,today_info_count,all_info_count,all_driver_count,today_driver_count,destination,id,departure,driving_length);
    }
  }
  //添加用户列表
  function addUserDisplay(cost,today_info_count,all_info_count,all_driver_count,today_driver_count,destination,id,departure,driving_length) {
    $(".userManage_container_ul").append('<li class="user_manage_container_li" user_id='+id+' onmouseleave="hideDeleteTip(this)">' +
            ...
            '<span class="userManage_span manage_driving_cost">'+cost+'</span>' +
            '<span class="userManage_span manage_cost">'+all_driver_count+'</span>' +
            '<span class="userManage_span manage_cost">'+today_driver_count+'</span>' +
            '<span class="userManage_span manage_cost">'+all_info_count+'</span>' +
            '<span class="userManage_span manage_cost">'+today_info_count+'</span>' +
...
//    event.preventDefault()
    ...
    $('.slide_container_top').find('.slide_container_top_title').text('添加线路');
  ...
  function showEditTip(obj){
    ...
    var route_cost =$(obj).parent().children('.manage_driving_cost').text();
    if(route_cost=="未定"){
      route_cost=""
    }
    ...
    $('.route_money').val(route_cost);
    $('.slide_container').animate({"top":"22%","opacity":"1"},300);
    $('.slide_container_top').find('.slide_container_top_title').text('编辑线路');
  ...
  <div id="ui_right">
    ...
    <div class="userManage_container">
      ...
      <ul class="userManage_container_ul">
        <li class="userManage_container_li_top">
          ...
          <span class="userManage_span">参考价</span>
          <span class="userManage_span">车主数量</span>
          <span class="userManage_span">今日车主数</span>
          <span class="userManage_span">发车数量</span>
          <span class="userManage_span">今日发车数</span>
        </li>
    ...
    <div class="slide_container">
      <div class="slide_container_top">
        <div class="slide_container_top">
          <span class="slide_container_top_title">添加线路</span>
      ...
      <div class="slide_container_mid">
        ...
        <div class="input_list">
          ...
          <input type="text" placeholder="参考价（元）" class="input_style route_money" />
        </div>
