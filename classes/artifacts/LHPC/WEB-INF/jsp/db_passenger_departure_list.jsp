<%--
  ...
  Date: 2016/8/24
  Time: 11:09
  describtion:后台管理--【APP信息管理】出行信息列表
...
<style>
  ...
  .driver_detail_option_edit, .driver_detail_option_delete, .driver_detail_option_QQ,.driver_detail_option_message {
    padding: 2px 8px;
    background-color: #F5AD4E;
    color: #fff;
    border-radius: 5px;
  ...
  .driver_detail_option_message{
    background-color: #3498db;
  }
  .driver_detail_option_message:hover {
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('.db_passenger_departure_list').addClass('active_li');
    // 绑定键盘按下事件
  ...
  var click_type;
  var user_id;
  var action_url = "/api/passenger/departure";
  ...
  function insertUserMessage() {
    for (var i = 0; i < global_data.data.length; i++) {
      ...
      var booking_seats = global_data.data[i].booking_seats;//可用座位
      var breakout_point = global_data.data[i].breakout_point;//目的地
      var boarding_point = global_data.data[i].boarding_point;//出发地
      ...
      addUserDisplay(insert_time,departure_city, destination_city,
              description, booking_seats,id, publish_time, departure, destination,boarding_point,breakout_point);
      if (departure == "") {
        $($('.begin_city')[i]).hide();
      }
      if (destination == "") {
  ...
  //添加用户列表
  function addUserDisplay(insert_time,departure_city, destination_city,
                          description, booking_seats,id, publish_time, departure, destination,boarding_point,breakout_point) {
    $(".not_find_message").before('<li class="drive_li_container" index=' + id + ' >' +
            ...
            '<span class="driver_type_span driver_type_seat">预定' + booking_seats + '个座位</span>' +
            '</div>' +
            '<div class="driver_detail_remark">' +
            '<div class="inline_block point_start">' +
            '<span>上车地点：</span>' +
            '<span style="color: #999" class="point_start_place">' + boarding_point + '</span>' +
            '</div>'+
            '<div class="inline_block point_end">' +
            '<span>下车地点：</span>' +
            '<span style="color: #999" class="point_end_place">' + breakout_point + '</span>' +
            '</div>'+
            '</div>' +
            '<div class="driver_detail_remark">' +
            '<span>备注信息：</span>' +
            ...
            '<span class="driver_detail_option_message" onclick="takeGo(this)">载ta一程</span>' +
  ...
  function addManagerStyle(obj) {
    ...
    var point_start_place = li.find('.point_start_place').text();
    var point_end_place = li.find('.point_end_place').text();
    ...
    $('.find_route_start').find('.find_route_span').text(point_start_place);
    $('.find_route_end').find('.find_route_span').text(point_end_place);
  ...
  function addCopyMessageStyle() {
    $('#message_container').append('<span style="display: inline-block;padding: 6px;">【来回拼车】牵起你生活中的每一次来回</span>'+
            ...
            '<div class="clear"></div>' +
            '</div>' +
            '<div class="input_list find_route_start">' +
            '<div class="input_list_left">' +
            '<span>【上车地点】</span>' +
            '</div>' +
            '<div class="input_list_right">' +
            ...
            '<div class="clear"></div>' +
            '</div>' +
            '<div class="input_list find_route_end">' +
            '<div class="input_list_left">' +
            '<span>【下车地点】</span>' +
            '</div>' +
            '<div class="input_list_right">' +
  ...
  function toEdit(obj) {
    ...
    window.location.href = "/db/passenger/departure?id=" + id;
  }
  function takeGo(obj) {
    var id = $(obj).closest('.drive_li_container').attr('index');
    window.location.href = "/db/passenger/departure?id=" + id;
  ...
  <div id="ui_right">
    ...
    <div class="userManage_container">
      ...
      <div class="sub_title_bar">
        ...
        <a href="/db/passenger/departure" class="add_span"><span
                class="add_classify">+</span>添加信息</a>
