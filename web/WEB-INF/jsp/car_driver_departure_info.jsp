...
<script type="text/javascript">
    ...
    function checkId() {
        ...
        var driver_id = url.split('driver_id=')[1].split('&order_id')[0];
        if(order_id==""){
            var action_url = "/api/driver/departure";
            $('.right_top_style').children('span').text('司机发车订单');
            updateMessage(driver_id,action_url);
        }else{
            var action_url = "/api/passenger/order_list";
            $('.right_top_style').children('span').text('乘客预定订单');
    ...
    function insertUserMessage() {
        ...
        var start_time = global_data.driver_data.start_time;//开始时间
        var mobile = global_data.driver_data.user_data.mobile;//手机号
        var departure_city = global_data.driver_data.boarding_point.city;//出发城市
        var destination_city = global_data.driver_data.breakout_point.city;//目的城市
        var departure = global_data.driver_data.boarding_point.address+global_data.driver_data.boarding_point.name;//出发小城
        var destination = global_data.driver_data.breakout_point.address+global_data.driver_data.breakout_point.name;//目的小城市
        var description = global_data.driver_data.description;
        var booking_count = global_data.driver_data.booking_count;//预定人数
        var inits_seats = global_data.driver_data.init_seats;//可用座位
        var car_brand = global_data.driver_data.user_data.car_brand;//车辆品牌
        var publish_time = global_data.driver_data.create_time;//id
        var user_avatar = global_data.driver_data.user_data.avatar;//微信头像
        var user_name = global_data.driver_data.user_data.name;//微信昵称
        var price = global_data.driver_data.price;
        if(user_avatar==""){
        ...
        addUserDisplay(booking_count,start_time,inits_seats, mobile, destination_city, departure_city, departure, destination, price,publish_time,
                car_brand,user_avatar, user_name, description);

        if (global_data.passenger_data.length == 0) {
            $('.passenger_container').append('<p class="not_manage">无乘客预定信息</p>');
        } else {
            for (var i = 0; i < global_data.passenger_data.length; i++) {
                var order_status = global_data.passenger_data[i].order_status; //0:待司机确认，1：司机 已确认，-1司机已拒绝，2，订单已失效
                var create_time = global_data.passenger_data[i].create_time;//开始时间
                var mobile = global_data.passenger_data[i].user_data.mobile;//手机号
                var departure_city = global_data.passenger_data[i].boarding_point.city;//出发城市
                var destination_city = global_data.passenger_data[i].breakout_point.city;//目的城市
                var departure = global_data.passenger_data[i].boarding_point.address + global_data.passenger_data[i].boarding_point.name;//出发小城
                var destination = global_data.passenger_data[i].breakout_point.address + global_data.passenger_data[i].breakout_point.name;//目的小城市
                var booking_seats = global_data.passenger_data[i].booking_seats;//预定人数
                var id = global_data.passenger_data[i].order_id;//id
                var user_avatar = global_data.passenger_data[i].user_data.avatar;//微信头像
                var user_name = global_data.passenger_data[i].user_data.name;//微信昵称
                var description = global_data.passenger_data[i].description;//微信昵称

                departure = departure_city+ " " + departure + " ";//出发小城
                destination = destination_city + " " + destination + " ";//目的小城市
                if (order_status == 1) {
                    order_status = "司机已确认"
                ...
                addPassengerDisplay(order_status, mobile, destination_city, departure_city, departure, destination, booking_seats, id,
                        user_avatar, user_name, description,insert_time);
            }
        }
    ...
    //添加车主列表
    function addUserDisplay(booking_count,start_time,inits_seats, mobile, destination_city, departure_city, departure, destination, price,publish_time,
                            car_brand,user_avatar, user_name, description) {
        $(".not_find_message").before('<li class="drive_li_container" index=' + 0+ ' >' +
                ...
                '<img src="/resource/images/pc_icon_car.png" class="driver_time_img">' +
                '<span class="driver_time_span driver_time_year">' + car_brand + '</span>' +
                '</div>' +
                ...
                '<span>发车时间：</span>' +
                '<span style="color: #999" class="driving_car_publish">' + start_time + '</span>' +
                '</div>' +
                '<div class="driver_detail_publish">' +
                '<span>发布时间：</span>' +
                ...
                '<div class="driver_detail_driver">' +
                '<span>预定人数：</span>' +
                '<span style="color: #f5ad4e" class="driving_car_mobile">' + booking_count + '人</span>' +
                '</div>' +
                '<div class="driver_detail_publish driver_money ">' +
                '<span>价格：</span>' +
                '<span style="color: #f5ad4e" class="driving_car_money">' + price + '</span>' +
                '</div>' +
                '<div class="clear"></div>' +
                '</div>' +
                '</div>' +
    ...
    //添加乘客列表
    function addPassengerDisplay(order_status, mobile, destination_city, departure_city, departure, destination, booking_seats, id,
                                 user_avatar, user_name, description,insert_time) {
        $(".passenger_container").append('<li class="passenger_container_li" index=' + id + '>' +
                ...
                '<span class="driver_type_span driver_type_time">' + insert_time + '</span>' +
                '</div>' +
                '</div>' +
                '<div class="passenger_message">' +
                '<div class="driver_detail_publish" style="float: left;">' +
                '<span>上车位置：</span>' +
                '<span style="color: #999" class="driving_car_mobile">'+departure+ '</span>' +
                '</div>' +
                '<div class="driver_detail_publish driver_breakout_point">' +
                '<span>下车位置：</span>' +
                '<span style="color: #999" class="driving_car_mobile">'+destination+ '</span>' +
                '</div>' +
                '<div class="clear"></div>' +
                '<div class="passenger_message_box">' +
                '<span>预定座位：</span>' +
                '<span style="color: #999" class="driving_car_publish">' + booking_seats + '</span>' +
                '</div>' +
                '<div class="passenger_message_box">' +
                '<span>联系方式：</span>' +
                '<span style="color: #999" class="driving_car_mobile">' + mobile + '</span>' +
                '</div>' +
                '<div class="driver_detail_publish" style="display: inline-block">' +
                '<span>备注信息：</span>' +
                '<span style="color: #999" class="driving_car_mobile">' + description + '</span>' +
                '</div>' +
                '<div class="driver_detail_publish driver_money " style="margin-right: 0">' +
                '<span>状态信息：</span>' +
                '<span style="color: #f5ad4e" class="driving_car_money">' + order_status + '</span>' +
                '</div>' +
                '<div class="clear"></div>' +
                '</div>' +
                '</li>'
            ...
            <div class="slide_container_mid">
                <div class="submit_select">
                    <span class="submit_select_span main_color submit_select_sure"
                          onclick="checkMessage()">确认已给车主转账</span>
