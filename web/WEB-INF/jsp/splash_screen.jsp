...
<script type="text/javascript">
  $(document).ready(function () {
    // 菜单切换
      ...
      $('#newsSplashScreen_menu').addClass('change_menu');
    load();
    swiper();
  });
  var i = 0;
  ...
  var global_id;
  var action_url = '/api/splash_screen/manage';
  var click_type=1;
  //请求列表数据
  function load(){
  ...
  function insertMessage(){
    for (var i = 0; i < global_data.slides.length; i++){
      var id=global_data.slides[i].id;//闪屏页id
      var content=global_data.slides[i].create_time.substring(0,16);//创建时间
      var title=global_data.slides[i].image_title;//标题
      var seq = global_data.slides[i].image_seq;//顺序
      ...
      console.log(global_data.slides[i])
  ...
  function ShopsListStyle(image_link,image_url,id, content, title, seq, subtitle) {
    $('.not_find_message').before('<li class="question_list question_list_message_box_right" id='+id+'>'+
            ...
            '<span class="article_tittle_box_span">闪屏排序</span>'+
  ...
  function addManagerStyle(){
    ...
    $('.slide_container_top').find('.slide_container_top_title').text('添加闪屏图');
  ...
  function showEditTip(obj){
    ...
    $('.ad_seq').text(img_seq);
    $('.slide_container').animate({"top":"12%","opacity":"1"},300);
    $('.slide_container_top').find('.slide_container_top_title').text('编辑闪屏图');
  ...
  function addAD() {
    ...
    data.append("weight", ad_seq);
    if(click_type == 2){
      data.append("id", global_id);
    }
    $.ajax({
      type: "post",
      url: "/api/splash_screen/manage",
      data: data,
      processData: false,
      contentType: false,
  ...
  <div id="ui_right">
    ...
    <div class="question_list_container">
      <div class="sub_title_bar">
        ...
        <span class="add_span" onclick="addManagerStyle()"><span class="add_classify">+</span>新建闪屏图</span>
        <div class="clear"></div>
      </div>
  ...
  <div class="slide_container">
    <form method="post" id="upload_form" accept-charset="utf-8" onsubmit="return false" enctype="multipart/form-data" action="/api/goods?action=create">
      <div class="slide_container_top">
        <span class="slide_container_top_title">添加闪屏</span>
