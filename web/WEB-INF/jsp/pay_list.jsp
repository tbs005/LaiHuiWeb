...
<script type="text/javascript">
  $(document).ready(function () {
      ...
      $('#fundCarPool_menu').addClass('change_menu');
    loadUser();
  ...
  function insertUserMessage() {
    ...
    var in_cash = global_data.in_cash;
    var out_cash = global_data.out_cash;
    $('.select_in').text('收入( '+in_cash+' 元)');
    $('.select_out').text('支出( '+out_cash+' 元)');
    for (var i = 0; i < global_data.data.length; i++) {
      ...
      if(pay_account==undefined || pay_account==""){
  ...
  function styleMessage(user_id, user_avatar, user_mobile, user_name, user_time, pay_type,pay_account,cash) {
    $(".not_find_message").before('<li class="user_container" index=' + user_id + ' onclick="toDetail(this)">' +
            ...
            '<p>付款金额：<span style="color:#f5ad4e">'+cash+' 元</span></p>' +
            '</div>' +
            '<div class="user_content" data_type='+pay_type+' data_account='+pay_account+' data_money='+cash+'>' +
            '<p>付款方式：'+pay_type+'</p>' +
//            '<p style="float: left;">提现账户：'+pay_account+'</p>' +
//            '<span class="delete_message" onclick="cashList(this)">完成转账</span>' +
            '<div class="clear"></div>' +
            '</div>' +
  ...
  function styleMessage2(user_id, user_avatar, user_mobile, user_name, user_time, pay_type,pay_account,cash) {
    $(".not_find_message").before('<li class="user_container" index=' + user_id + ' onclick="toDetail(this)">' +
            ...
            '</div>' +
            '<div class="user_content">' +
            '<p>提现金额：<span style="color:#f5ad4e">'+cash+' 元</span></p>' +
            '</div>' +
            '<div class="user_content" data_type='+pay_type+' data_account='+pay_account+' data_money='+cash+'>' +
            '<p>提现方式：'+pay_type+'</p>' +
            '<p style="float: left;">提现账户：'+pay_account+'</p>' +
//            '<span class="delete_message2">已完成</span>' +
  ...
  function changeDataStyle(time){
    ...
    $('.select_start_time ').val(end_time+" 00:00");
    $('.select_end_time ').val(end_time+" 23:59");
  }

  //获取点击id并跳转
  function toDetail(obj) {
    var id=$(obj).attr('index');
    window.location.href = "/pay/info?order_id="+id+"&end";
  }
...
<div class="ui_body">
  ...
  <div id="ui_right">
    ...
    <div class="userManage_container">
      ...
      <div class="user_list_drop_box" style="margin-left: 10px">
      ...
      <div class="sub_title_bar">
        ...
        <div class="publish_time_select">
          <div class="select_checkbox_container select_checkbox_active" onclick="addTabStyle(this)">
            <span class="select_checkbox select_in" index="">收入( 0 元)</span>
            <i class="select_popover_arrow"></i>
          </div>
          <div class="select_checkbox_container" onclick="addTabStyle(this)">
            <span class="select_checkbox select_out" index="1">支出( 0 元)</span>
            <i class="select_popover_arrow"></i>
          </div>
        </div>
        <div class="page_box">
