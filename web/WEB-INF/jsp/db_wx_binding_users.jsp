...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('.db_wx_binding_users').addClass('active_li');
    // 绑定键盘按下事件
  ...
  function insertUserMessage(){
    ...
    for (var i = 0; i < global_data.data.length; i++){
      ...
      var time = global_data.data[i].create_time.substring(0,19);
      if(user_avatar==""){
        user_avatar="/resource/images/icon_people.png";
      }
      addUserDisplay(user_avatar,user_id,user_mobile,user_name,time);
    }
  }
  //添加用户列表
  function addUserDisplay(user_avatar,user_id,user_mobile,user_name,time) {
    $(".userManage_container_clear").before('<li class="weixin_conyainer" index='+user_id+' onclick="toDetail(this)">'+
            ...
            '<span>加入时间</span>'+
            '<span class="count_style">'+time+'</span>'+
            '</div>'+
            '</div>'+
            '<div class="clear"></div>'+
            '</li>')
  ...
  function toDetail(obj){
    window.location.href="/db/car_owner/base_info?id="+$(obj).attr('index');
  }
  //显示下拉
