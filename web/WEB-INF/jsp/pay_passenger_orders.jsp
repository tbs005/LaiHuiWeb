...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('.pay_passenger_orders').addClass('active_li');
    // 绑定键盘按下事件
  ...
  function findMessage(){
    var obj={};
//    var order_id = $('.driver_span').attr('cat_id');
    var search = $('.search_user_input').val();
//    var start_time = $('.select_start_time ').val();
//    var end_time = $('.select_end_time ').val();
    var publish_status = $('.publish_status').attr('index');
    obj.action='show';
    obj.keyword=search;
    obj.status=publish_status;
    obj.size=size;
    obj.page=global_page;
    operation.operation_ajax(action_url,obj,sendMessage);
  }
  ...
  function insertUserMessage(){
    ...
    for (var i = 0; i < global_data.data.length; i++){
      var pay_num = global_data.data[i].pay_num;
      var order_name = global_data.data[i].user_name;
      var order_mobile = global_data.data[i].mobile;
      var order_time = global_data.data[i].create_time.substring(5,16);
      var order_count_money = global_data.data[i].money;
      var order_driver_id = global_data.data[i].order_id;
      ...
      addUserDisplay(pay_num,order_name,order_mobile,order_time,order_count_money,order_tips,order_driver_id);
  ...
  //添加用户列表
  function addUserDisplay(pay_num,order_name,order_mobile,order_time,order_count_money,order_tips,order_driver_id) {
    $(".userManage_container_ul").append('<li class="user_manage_container_li" user_id='+order_driver_id+'>' +
            '<span class="userManage_span pay_num">'+pay_num+'</span>' +
            '<span class="userManage_span ">'+order_name+'</span>' +
            '<span class="userManage_span ">'+order_mobile+'</span>' +
            '<span class="userManage_span">'+order_time+'</span>' +
            '<span class="userManage_span">'+order_count_money+'</span>' +
            '<span class="userManage_span order_id" onclick="toDetail(this)">'+order_driver_id+'</span>' +
  ...
  function toDetail(obj) {
    var order_id=$(obj).parent().attr('user_id');
    window.location.href = "/db/pay/d_orders?order_id=" + order_id+"&end";
  }
</script>
<%--右侧菜单--%>
<div class="ui_body">
    ...
    <div class="userManage_container">
      ...
      <ul class="userManage_container_ul">
        <li class="userManage_container_li_top">
          <span class="userManage_span pay_num_title">流水号</span>
          <span class="userManage_span">乘客姓名</span>
          <span class="userManage_span">手机号</span>
          <span class="userManage_span">订单创建时间</span>
          <span class="userManage_span">支付金额</span>
          <span class="userManage_span">支付车单号</span>
