 ...
 */
public class TotalPayOrderMapper implements RowMapper<PayOrder> {
    ...
    public PayOrder mapRow(ResultSet resultSet, int i) throws SQLException {
        ...
        pay.setUser_mobile(resultSet.getString("user_mobile"));
        pay.setOrder_id(resultSet.getInt("order_id"));
        pay.setPay_money(resultSet.getDouble("total_pay_money"));
        pay.setPay_num(resultSet.getString("all_pay_num"));
        pay.setCreate_time(resultSet.getString("last_updated_time").split("\\.")[0]);
