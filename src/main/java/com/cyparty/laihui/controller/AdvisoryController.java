...
public class AdvisoryController {
        ...
        try {
            ...
            switch (action) {
                case "add":
                    ...
                    //判断是更新还是创建
                    String adversoty_id = request.getParameter("id");
                    if (adversoty_id != null && !adversoty_id.trim().equals("")) {
                        id = Integer.parseInt(adversoty_id);
                    }
                    if (id > 0) {
                        //修改
                        ...
                        String update_sql = " set title='" + Utils.checkNull(title) + "',sub_title='" + Utils.checkNull(sub_title) + "',content='" + Utils.checkNull(content) + "',update_date='" + Utils.getCurrentTime() + "' where advisory_id=" + id;
    ...
    @ResponseBody
    @RequestMapping(value = "/advisory/manage/upload", method = RequestMethod.POST)
    public String add(HttpServletRequest request, @RequestParam("myFileName") MultipartFile myfile) {
        HttpHeaders responseHeaders = new HttpHeaders();
        responseHeaders.set("Content-Type", "application/json;charset=UTF-8");
        responseHeaders.set("Access-Control-Allow-Origin", "*");
        JSONObject result = new JSONObject();
        String upload = fileService.uploadFile(request, myfile, "upload");
        if (upload != null) {
            return upload;
        } else {
            return null;
